...
//        search(MOVIES,"Movie #1000", "name");
//        endTime = System.currentTimeMillis();
//        System.out.println(("Search for all fields by any field value: "+(endTime-startTime)+" millis"));
        FuzzySearch levenshtein = new Levenshtein();
        System.out.println(levenshtein.getDistance("hello", "malloc"));
        FuzzySearch jaroWinkler = new JaroWinkler();
        System.out.println(jaroWinkler.getDistance("hello", "malloc"));
        FuzzySearch damerauLevenshtein = new DamerauLevenshtein();
        System.out.println(damerauLevenshtein.getDistance("hello", "malloc"));
        System.out.println(jaroWinkler.fits("hello", "malloc", 0.6));
    ...
    public void run2() {
        ...
        startTime = System.currentTimeMillis();
        fuzzySearch(MOVIES,"Movie #1","name",levenshtein,3);
        endTime = System.currentTimeMillis();
        System.out.println(("Search levenshtain: "+(endTime-startTime)/1000+" seconds"));

        startTime = System.currentTimeMillis();
        fuzzySearch(MOVIES,"Movie #1","name",jaroWinkler,0.6);
        endTime = System.currentTimeMillis();
        System.out.println(("Search jaro-winkler: "+(endTime-startTime)/1000+" seconds"));

        startTime = System.currentTimeMillis();
        fuzzySearch(MOVIES,"Movie #1","name",damerauLevenshtein,3);
        endTime = System.currentTimeMillis();
        System.out.println(("Search jaro-winkler: "+(endTime-startTime)/1000+" seconds"));
...
//        System.out.println(data.size() + " rows selected.\n");
    ...
    public ArrayList<Map<String, String>> fuzzySearch(String Category, String SearchKey, String Field, FuzzySearch fs, double limit) {
        // result data array
        ArrayList<Map<String, String>> data = new ArrayList<Map<String, String>>();
        // translated query
        String translatedSearchKey = langpadSwitch(SearchKey);
...
//        System.out.println("  where field \"" + Field + "\" contains \""+SearchKey+"\" (or maybe \""+translatedSearchKey+"\")\n");
        ...
        if (!(Category.equalsIgnoreCase(MOVIES) || Category.equalsIgnoreCase(ACTORS))) {
            ...
            int count = 0;
            for (String id : idlist) {
                //System.out.println("Items processed out of " + idlist.size() + ": " + count++);
                value = jedis.hget(name(Category, id), Field);

                // hashmap handling
                contains = fs.fits(value, SearchKey, limit);
